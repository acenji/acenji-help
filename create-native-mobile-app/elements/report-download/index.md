...
The Report Download element allows users to generate and download PDF reports containing data from your mobile application. This element enables you to configure multiple data sources, select specific fields to include, and choose whether to include data from the current user only or all users. The Report Download element is available for **Mobile Cards**.
...
- **Current User Only:** The report will only contain data entered by the user who clicks the download button. This is ideal for personal reports or individual progress tracking.
- **All Users:** The report will aggregate data from all users who have interacted with the selected sources. This is useful for managers or administrators who need to review data across the entire user base.
...
This allows you to build comprehensive reports that pull data from different parts of your application.
...
When using the Report Download element in mobile applications:

1. **File Handling:** On mobile devices, the PDF will be downloaded and can be opened with the device's default PDF viewer or shared via standard sharing options.

2. **Network Requirements:** Report generation requires an active internet connection as the PDF is generated server-side.

3. **Loading Feedback:** The loading text is especially important on mobile to let users know the report is being prepared, as generation may take a few seconds depending on data volume.
...
5. **Optimize for mobile:** Keep button text concise and ensure the report download button is easily tappable on smaller screens.
...
<span class="triangle"></span> What is the Button Element? - https://help.acenji.com/#/./create-native-mobile-app/elements/button/index
<span class="triangle"></span> Using the Card List Element - https://help.acenji.com/#/./create-native-mobile-app/elements/card-list/index
<span class="triangle"></span> Learn about the Info Element - https://help.acenji.com/#/./create-native-mobile-app/elements/info/index
